...
final class MOMMessageTests: XCTestCase {
  ...
  func testEncodeReplyMixedParams() {
    // Hardware MOM discovery-reply wire order.
    let bytes = MOMMessage.encode(
      .enumerateDevices | .typeDeviceReply,
      params: [.int(0), .int(1), .int(1), .string("DAD-MOM"),
               .string("710"), .int(0), .string("71000000000")]
    )
    XCTAssertEqual(bytes,
                   Data(":edev,0,1,1,'DAD-MOM','710',0,'71000000000'\r".utf8))
  }
  ...
  func testRoundTripNotification() {
    ...
    let params: [MOMParameter] = [.int(2), .string("foo")]
